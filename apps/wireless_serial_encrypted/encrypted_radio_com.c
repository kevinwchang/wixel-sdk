...
static void receiveMorePackets(void)
{
    ...
    while(packet = radioLinkRxCurrentPacket())
    {
        ...
        case PAYLOAD_TYPE_DATA:
            // first byte is length; decrypt 1 block (16 bytes) after that
            aesDecrypt(packet + 1, packet + 1, 1);
            ...
            rxBytesLeft = AES_BLOCK_SIZE - packet[1 + AES_BLOCK_SIZE - 1]; // Compute the data length. The last byte contains the number of padding bytes.
